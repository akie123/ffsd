...
document.getElementById("but-4").addEventListener("click", async function(e) {
    ...
    let fl = 0;
    let str = ''
    let na = document.getElementById('namedoc');
    if(na.value == ''){
        str = str + 'Name field cannot be empty\n';
        fl = 1;
    }
    ...
    let dob = document.getElementById('dobd');
    if(dob.value == ''){
        str = str + 'Date of birth cannot be empty\n';
        fl = 1;
    }

    let g = document.getElementById('genderD');
    if(g.value == ''){
        str = str + 'Gender cannot be empty\n';
        fl = 1;
    }

    let ad = document.getElementById('adhar-card-d');
    if(ad.value == ''){
        str = str + 'Aadhaar Number cannot be empty\n';
        fl = 1;
    }

    else if(ad.value.length != 12){
        str = str + 'Aadhaar Number must have 12 digits\n';
        fl = 1;
    }

    for(let i=0;i<ad.value.length;i++){
        let temp = ad.value.charCodeAt(i);
        if(temp<=48 && temp>=57)
        {
    ...
    let qua = document.getElementById('qualification');
    if(qua.value == ''){
        str = str + 'Qualification cannot be empty\n';
        fl = 1;
    }

    let dep = document.getElementById('department');
    if(dep.value == ''){
        str = str + 'Department cannot be empty\n';
        fl = 1;
    }

    let exp = document.getElementById('exp');
    if(exp.value == ''){
        str = str + 'Experience cannot be empty\n';
        fl = 1;
    }
